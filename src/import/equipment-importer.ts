...
function buildEquipmentState(engines: DemiplaneEngineEntry[]): EquipmentState {
  ...
  return {
    primaryHandId: findCustom("character_hand_primary_equipped-id") as
      string | undefined,
    offHandId: findCustom("character_hand_offhand_equipped-id") as
      string | undefined,
    bothHandsId: findCustom("character_hand_both_equipped-id") as
      string | undefined,
    wornIds,
    containerMap,
    quantityMap,
...
function resolveEquippedState(
  ...
  if (state.primaryHandId === demiplaneId)
    return { carryType: "held", handsHeld: 1, invested: null };
  if (state.offHandId === demiplaneId)
    return { carryType: "held", handsHeld: 1, invested: null };
  if (state.bothHandsId === demiplaneId)
    return { carryType: "held", handsHeld: 2, invested: null };
  if (state.containerMap.has(demiplaneId))
...
function findBySlug(
  equipIndex: {
    find: (
      fn: (e: { system?: { slug?: string } }) => boolean,
    ) => { _id: string } | undefined;
  },
  slug: string,
): { _id: string } | undefined {
  const exact = equipIndex.find(
    (e: { system?: { slug?: string } }) => e.system?.slug === slug,
  );
  if (exact) return exact;

  const fallbackSlug = slug.replace(
    /-(basic|lesser|greater|moderate|major|superb)$/,
    "",
  );
  if (fallbackSlug !== slug) {
    return equipIndex.find(
...
async function createBackpackFirst(
  ...
  const backpackIdx = items.findIndex(
    (i) => (i.data.type as string) === "backpack",
  );
  ...
  for (const item of items) {
    if (
      state.containerMap.get(item.demiplaneId) === backpackEntry.demiplaneId
    ) {
      (item.data.system as Record<string, unknown>).containerId =
        backpackFoundryId;
    }
  }
  return 1;
...
export async function applyEquipment(
  ...
  for (const eng of itemEngines) {
    const slug = normalizeEquipmentSlug(
      (eng.args?.slug as string) ?? eng.name.split("/").pop() ?? "",
    );
  ...
  if (items.length === 0) {
    if (skipped.length > 0)
      summary.log.push(`! equipment: ${skipped.length} items not found`);
    return;
  }
...
export async function applyCurrency(
  ...
  for (const { engine, slug } of CURRENCY_MAP) {
    const eng = engines.find(
      (e) => e.type === "CustomDemiplaneEngine" && e.name === engine,
    );
  ...
  if (coinItems.length > 0) {
    ...
    const desc = coinItems
      .map((c) => `${(c.system as { quantity: number }).quantity} ${c.name}`)
      .join(", ");
