...
| feature-spell-resolver | Stream-engines (feature/feat definitions carrying `add-spell` mods, plus the spellcasting-subtree chase for automatic grants with no character engines) | Innate / Focus / Hexes / apparition entries, or repertoire additions       |
...
**Grant categorization** (`feature-spell-resolver`) sorts each `add-spell` modifier into exactly one bucket — innate, hex, apparition, known (repertoire), or focus — via a small set of predicates. Order matters: a witch feature grants a hex (Needle of Vengeance) _and_ a plain familiar spell (Phantom Pain) from one engine, so the hex is tested first (it carries the `hex-spells` focus group or, for a witch, focus-casting machinery — a `saveDC`/`spellAttack`) and the plain spell falls through to the repertoire. The machinery test is gated on the class actually declaring a hex focus group, so a non-witch focus grant with the same shape (e.g. a sorcerer bloodline spell) is not misfiled as a hex. A grant with an absent tradition is treated the same as `"inherit"` (both mean "the class's own tradition"), so Phantom Pain — whose grant omits the field — still joins the witch's prepared list. A grant parented at the class's main spellcasting feature (an oracle mystery's ignition / breathe fire, the necromancer's harm) is repertoire regardless of tradition — only focus-group-parented grants are focus. The one shape the grant alone cannot decide is a repertoire-shaped grant (`isKnown`, concrete tradition, no focus group) for a spell that is itself a focus spell (the necromancer's grave cantrips, shaped exactly like the bard's Soothe): those are split by the spell definitions' own `isFocus` flags (the cantrips don't even carry the "focus" trait), with unresolvable definitions keeping the repertoire default.
